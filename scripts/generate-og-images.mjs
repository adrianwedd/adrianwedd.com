...
async function generateForDir(dir, label, generatedSlugs) {
  ...
  for (const file of files) {
    ...
    if (fs.existsSync(outPath)) {
      if (generatedSlugs.has(slug)) {
        console.log(`  [${label}] skipped (collision): ${slug}.png already generated by ${generatedSlugs.get(slug)}`);
      }
      skipped++;
      continue;
    }
...
async function main() {
  ...
  const projects = await generateForDir(PROJECTS_DIR, 'Project', generatedSlugs);
  const blog = await generateForDir(BLOG_DIR, 'Blog', generatedSlugs);
