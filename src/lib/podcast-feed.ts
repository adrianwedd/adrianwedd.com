...
export interface FeedOptions {
  /**
   * Drop entries that only have a videoUrl. Spotify refuses any feed carrying a
   * video enclosure ("We're unable to accept podcasts with videos"); Apple and
   * generic RSS readers accept them, so the default feed keeps them.
   */
  audioOnly?: boolean;
  /** Path this feed is served from, for the required self-referencing atom:link. */
  selfPath: string;
}

export async function buildPodcastFeed(context: APIContext, options: FeedOptions) {
  const { audioOnly = false, selfPath } = options;
  const [episodes, blogs, projects] = await Promise.all([
    getCollection('audio').then((all) =>
      all
        .filter((e) => !e.data.draft && (!audioOnly || !!e.data.audioUrl))
