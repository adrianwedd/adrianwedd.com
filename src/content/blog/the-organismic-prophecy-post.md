...
This post is about what happens when that budget collapses, and why the same vocabulary — _prediction_, _error_, _hallucination_, _attention_ — does not mean the same thing when we use it about a language model. The metaphor has eaten the discourse. It's worth saying out loud where it stops working.
