...
Risk levels cascade: CRITICAL if any critical indicator is present, HIGH if two or more high indicators, MEDIUM if one high or two medium, LOW otherwise. The logic is hardcoded and auditable, ensuring safety protocols are not mediated by a language model.
