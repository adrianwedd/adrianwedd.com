...
They're actually the same thread.
...
For a kid with a Pathological Demand Avoidance profile, the unreliable behaviour is the demand-detection response. Not unreliable in some judgemental sense — the kid is fine; the response is what it is. Demand-avoidance is often described as a state where any input the nervous system reads as a directive triggers a refusal that arrives before the directive has been consciously processed. It functions like a neurological tripwire rather than a deliberative choice. Asking the kid to "be more compliant" is the same kind of category error as asking a language model to "be more aligned": the system isn't engineered to reach the desired behaviour from that direction. The behaviour you want has to come from somewhere else.
...
Non-coercive design moves the locus of safety into the environment. It's a structural property of the deployment, not a behavioural property of the component. The model can be jailbroken; the architecture can't. The child can be in meltdown; the Three S's protocol still runs. The component is best-effort. The structure is authoritative.
...
The next AI deployment you're asked to safety-review, run the SPARK test on it. Strip the model out for a second. Look at what the architecture *enforces*, not what the model *promises*. If the architecture would still produce safe behaviour even with the model jailbroken, you're looking at a non-coercive design. If the safe behaviour depends on the model deciding to behave, you're looking at coercion in a tuxedo, and the failure modes are already named and waiting in literature you haven't read yet.
