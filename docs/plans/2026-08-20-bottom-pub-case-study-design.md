...
`A failed bid to buy a Tasmanian pub left behind a frozen civic archive, a privacy pipeline that can't leak, audited charts and a release system that distrusts itself.` (159 — recount before commit)
...
- **Exhibit 10 · v1 → v2, and the concept that didn't ship.** Side-by-side: v1
  ...
  generate_visual_prototypes_v2.py"; dek: _"every varying visual channel now computed from
