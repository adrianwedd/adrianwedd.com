...
test('paginated listing renders the pager and prev navigates back', async ({ page }) => {
  ...
  // The pager sits at the document bottom — the same below-the-fold geometry
  // clickCard exists for on the nightly mobile project.
  await clickCard(page, pager.locator('a[rel="prev"]'));
